...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd,
	int screenWidth, int screenHeight, CXMMATRIX baseViewMatrix)
{
	bool result;
...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, CXMMATRIX worldMatrix, CXMMATRIX orthoMatrix)
{
	bool result;
	...
	float red, float green, float blue, ID3D11DeviceContext* deviceContext)
{
	...
	{
		if(text[i] != ' ') drawLetters++;
	}
	sentence->drawCount = drawLetters * 6;
...
bool TextClass::RenderSentence(ID3D11DeviceContext* deviceContext, SentenceType* sentence, CXMMATRIX worldMatrix,
							   CXMMATRIX orthoMatrix)
{
	...
	result = m_FontShader->Render(deviceContext, sentence->drawCount, worldMatrix, m_baseViewMatrix,
...
bool TextClass::SetObjectCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	strcpy_s(str, "Objects = ");
	strcat_s(str, temp);
	return UpdateSentence(m_sentence3, str, 20, 80, 1.0f, 1.0f, 1.0f, deviceContext);
}
...
bool TextClass::SetPolygonCount(int count, ID3D11DeviceContext* deviceContext)
{
	char temp[32], str[32];
	_itoa_s(count, temp, 10);
	strcpy_s(str, "Polygons = ");
	strcat_s(str, temp);
	return UpdateSentence(m_sentence4, str, 20, 100, 1.0f, 1.0f, 1.0f, deviceContext);
}

bool TextClass::SetResolution(int width, int height, ID3D11DeviceContext* deviceContext)
{
	char wStr[32], hStr[32], str[32];
	_itoa_s(width,  wStr, 10);
	_itoa_s(height, hStr, 10);
	strcpy_s(str, wStr);
	strcat_s(str, " x ");
	strcat_s(str, hStr);
	return UpdateSentence(m_sentence5, str, 20, 120, 1.0f, 1.0f, 1.0f, deviceContext);
}